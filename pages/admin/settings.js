            ...
            <>
              {activeTab === "cs" && (
                <>
                  <div style={{ ...cardStyle, borderLeft: "4px solid var(--accent)" }}>
                    ...
                    {activeCS && (
                      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        <div style={{ width: 48, height: 48, borderRadius: "50%", background: "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontFamily: "var(--font-display)", flexShrink: 0 }}>
                        ...
                        {testHour !== null && (
                          <button onClick={() => setTestHour(null)} style={{ marginLeft: "auto", fontSize: 12, color: "var(--accent)", background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>
                            Reset ke sekarang
                          </button>
                        )}
                      </div>
                      ...
                      return (
                        <div key={cs} style={{ marginBottom: 10 }}>
                          ...
                          <div style={{ position: "relative", height: 32, background: "var(--bg-alt)", borderRadius: 6, overflow: "hidden" }}>
                            <div style={{ position: "absolute", left: `${(start/24)*100}%`, width: `${(dur/24)*100}%`, top: 0, bottom: 0, background: isActive ? "var(--accent)" : "rgba(201,150,138,0.3)", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.3s" }}>
                    ...
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {Array.from({ length: 24 }, (_, i) => i).map((h) => (
                          <button key={h} onClick={() => setTestHour(h)} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", border: "1.5px solid", borderColor: testHour === h ? "var(--ink)" : "var(--border)", background: testHour === h ? "var(--ink)" : "transparent", color: testHour === h ? "#fff" : "var(--ink)", fontSize: 12, cursor: "pointer" }}>
                  ...
                  {["cs1","cs2"].map((cs, idx) => (
                    <div key={cs} style={cardStyle}>
                      <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: idx === 0 ? "var(--accent)" : "#8A968A", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 13, fontWeight: 700 }}>
                          {settings[`${cs}_name`].split(" ").pop()?.[0] || (idx+1)}
                        </div>
                        CS {idx + 1}
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                        <div><label style={labelStyle}>Nama</label><input style={inputStyle} value={settings[`${cs}_name`]} onChange={(e) => handleChange(`${cs}_name`, e.target.value)} /></div>
                        <div><label style={labelStyle}>Nomor WA (628xxx)</label><input style={inputStyle} value={settings[`${cs}_phone`]} onChange={(e) => handleChange(`${cs}_phone`, e.target.value)} /></div>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        <div>
                          <label style={labelStyle}>Jam Mulai Shift</label>
                          <select style={inputStyle} value={settings[`${cs}_shift_start`]} onChange={(e) => handleChange(`${cs}_shift_start`, e.target.value)}>
                            {Array.from({ length: 24 }, (_, i) => <option key={i} value={i}>{String(i).padStart(2,"0")}:00</option>)}
                          </select>
                        </div>
                        <div>
                          <label style={labelStyle}>Jam Selesai Shift</label>
                          <select style={inputStyle} value={settings[`${cs}_shift_end`]} onChange={(e) => handleChange(`${cs}_shift_end`, e.target.value)}>
                            {Array.from({ length: 24 }, (_, i) => i+1).map((i) => (
                              <option key={i} value={i}>{i === 24 ? "00:00 (tengah malam)" : `${String(i).padStart(2,"0")}:00`}</option>
                            ))}
                          </select>
                        </div>
                      </div>
                  ...
                  <div style={cardStyle}>
                    ...
                    <p style={{ fontSize: 12, color: "var(--ink-light)", marginTop: 8 }}>CS ini menerima chat jika tidak ada yang sedang shift.</p>
                  </div>
                </>
              )}
              ...
              {activeTab === "pixel" && (
                <div style={cardStyle}>
                  ...
                  <p style={{ fontSize: 13, color: "var(--ink-light)", marginBottom: 20, lineHeight: 1.6 }}>
                    Pixel ID untuk tracking event Meta Ads — PageView, ViewContent, Contact, InitiateCheckout.
                  </p>
