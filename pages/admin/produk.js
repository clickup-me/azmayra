...
async function analyzeProductImage(imageUrl, existingName) {
  const response = await fetch("/api/analyze-image", {
    ...
    body: JSON.stringify({ imageUrl: imageUrl, existingName: existingName }),
  });
...
function ProductForm({ initial, onSave, onCancel }) {
  ...
  const [uploadedUrls, setUploadedUrls] = useState(
    initial && initial.images ? initial.images.split("\n").filter(Boolean) : []
  );
  ...
  async function handleFileUpload(e) {
    ...
    if (!CLOUD_NAME) {
      alert("Cloudinary belum dikonfigurasi.");
      return;
    }
    setUploading(true);
    setError("");
    try {
      ...
      setUploadedUrls(function(prev) { return prev.concat(urls); });
      setForm(function(prev) {
        const existing = prev.images ? prev.images.split("\n").filter(Boolean) : [];
        return Object.assign({}, prev, { images: existing.concat(urls).join("\n") });
      });
    } catch (err) {
  ...
  async function handleAnalyze() {
    ...
    if (!lines.length) { alert("Upload foto dulu sebelum analisis."); return; }
    setAnalyzing(true);
    setError("");
    try {
      const result = await analyzeProductImage(lines[0].trim(), form.name);
      setForm(function(prev) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      var colors = [];
      try { colors = JSON.parse(form.colors); } catch(e) { colors = []; }
      const payload = {
        slug: form.slug.trim(),
        name: form.name.trim(),
        ...
        sizes: form.sizes ? form.sizes.split(",").map(function(s) { return s.trim(); }).filter(Boolean) : [],
        colors: colors,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ background: "var(--bg-alt)", borderRadius: "var(--radius-md)", padding: 20 }}>
        ...
        {uploadedUrls.length > 0 && (
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
            {uploadedUrls.map(function(url, i) {
              return (
                <div key={i} style={{ position: "relative" }}>
                  ...
                  <button type="button" onClick={function() {
                    const newUrls = uploadedUrls.filter(function(_, idx) { return idx !== i; });
                    setUploadedUrls(newUrls);
                    setForm(function(prev) { return Object.assign({}, prev, { images: newUrls.join("\n") }); });
                  }} style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: "50%", background: "#E24B4A", color: "#fff", border: "none", fontSize: 12, cursor: "pointer" }}>x</button>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={function() { fileRef.current && fileRef.current.click(); }} disabled={uploading} style={{
        ...
        <div style={{ marginTop: 12 }}>
          <label style={Object.assign({}, labelStyle, { marginBottom: 4 })}>Atau paste URL foto (satu per baris)</label>
          <textarea style={Object.assign({}, inputStyle, { resize: "vertical", fontSize: 12, fontFamily: "monospace" })}
      ...
      <div>
        ...
        <textarea style={Object.assign({}, inputStyle, { fontFamily: "monospace", fontSize: 12, resize: "vertical" })}
          name="colors" value={form.colors} onChange={handleChange} rows={3}
          placeholder='[{"name":"Sage","hex":"#B2BEA5"}]' />
      </div>
      <div><label style={labelStyle}>Deskripsi</label><textarea style={Object.assign({}, inputStyle, { resize: "vertical" })} name="description" value={form.description} onChange={handleChange} rows={3} /></div>
      <div>
        <label style={labelStyle}>Detail Produk (satu per baris)</label>
        <textarea style={Object.assign({}, inputStyle, { resize: "vertical" })} name="details" value={form.details} onChange={handleChange} rows={4} placeholder="Bahan: Linen premium" />
      </div>
      <div><label style={labelStyle}>Pesan WA Otomatis</label><textarea style={Object.assign({}, inputStyle, { resize: "vertical" })} name="wa_message" value={form.wa_message} onChange={handleChange} rows={2} /></div>
...
export default function AdminProduk() {
  ...
  async function loadProducts() {
    setLoading(true);
    const res = await supabase.from("products").select("*").order("sort_order");
  ...
  function handleEdit(product) {
    setEditing(Object.assign({}, product, {
          ...
          {mode === "list" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
                ...
                <button onClick={function() { setMode("add"); }} style={{
                    ...
                    return (
                      <div key={p.id} style={{
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={function() { handleEdit(p); }} style={{ padding: "8px 16px", border: "1.5px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: 13, cursor: "pointer", background: "transparent" }}>Edit</button>
                          {deleteId === p.id ? (
                            <>
                              <button onClick={function() { handleDelete(p.id); }} style={{ padding: "8px 16px", background: "#E24B4A", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", fontSize: 13, cursor: "pointer" }}>Yakin hapus?</button>
                              <button onClick={function() { setDeleteId(null); }} style={{ padding: "8px 16px", border: "1.5px solid var(--border)", borderRadius: "var(--radius-sm)", fontSize: 13, cursor: "pointer", background: "transparent" }}>Batal</button>
                            </>
                          ) : (
                            <button onClick={function() { setDeleteId(p.id); }} style={{ padding: "8px 16px", border: "1.5px solid #E24B4A", color: "#E24B4A", borderRadius: "var(--radius-sm)", fontSize: 13, cursor: "pointer", background: "transparent" }}>Hapus</button>
                          )}
                        </div>
                      </div>
                    );
          ...
          {mode === "add" && (
            <>
              ...
              <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", padding: 28, boxShadow: "var(--shadow-card)" }}>
                <ProductForm onSave={handleSaved} onCancel={function() { setMode("list"); }} />
              </div>
            </>
          )}
          {mode === "edit" && editing && (
            <>
              ...
              <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", padding: 28, boxShadow: "var(--shadow-card)" }}>
                <ProductForm initial={editing} onSave={handleSaved} onCancel={function() { setMode("list"); setEditing(null); }} />
              </div>
            </>
          )}
        </div>
